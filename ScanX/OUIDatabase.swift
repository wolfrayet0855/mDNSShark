...
/// A singleton for looking up OUIs from a static in-code dictionary of unique entries.
class OUIDatabase {
    static let shared = OUIDatabase()
    
    private let logger = Logger(subsystem: "com.example.ScanX", category: "OUIDatabase")
    
    // Store all OUI prefixes in lowercase, so we can do a simple `.lowercased()` check.
    // The Apple block now has 31 entries total.
    ...
    /// Looks up the manufacturer for a given OUI (e.g. "a4:cf:99"),
    /// ignoring upper/lower casing in the input.
